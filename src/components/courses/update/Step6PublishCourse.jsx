...
const Step6PublishCourse = ({ data, updateData, onPrevious }) => {
  ...
  const { watch, register, reset, control } = form;

  useEffect(() => {
    if (data?.publishInfo) {
      const { instructors = [], status } = data.publishInfo;

      reset({
        instructors: instructors?.map((instructor) => instructor._id),
        status,
      });
    }
  }, [data?.publishInfo]);
  ...
  console.log("selectedInstructorIds", selectedInstructorIds);
  const selectedInstructorDetails = instructors.filter((instructor) =>
    selectedInstructorIds.includes(instructor._id)
  );
  console.log("selectedInstructorDetails", selectedInstructorDetails);
